...
- **발행 시각 랜덤화**: frontmatter `date` 필드는 정시(09:00, 14:00 등)를 절대 사용하지 말고 분 단위까지 무작위로 설정 (예: `09:51`, `18:23`, `02:34`). 시각은 00:00~23:59 전 범위에서 균등 분포로 선택, 같은 날 여러 글 발행 시도 서로 다른 시각 사용. 이유: SEO 양산 패턴 신호를 줄여 신생 도메인의 색인률 개선 (Google 헬프풀 콘텐츠 알고리즘 회피). 빌드 cron도 비대칭 시각 6회로 설정되어 있으므로 글 작성 후 최대 4시간 이내 자동 노출됨.
